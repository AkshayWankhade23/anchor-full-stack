...
const Home = () => {
  ...
  React.useEffect(() => {
    ...
    if (email) {
      fetch(`https://anchor-full-stack.onrender.com/get-coins?email=${email}`)
        .then((res) => res.json())
  ...
  React.useEffect(() => {
    setIsLoading(true);
    fetch("https://anchor-full-stack.onrender.com/internships")
      .then((res) => res.json())
      .then((data) => {
        setJobs(data);
        console.log(data)
  ...
  const handleapplyJobs = (e) => {
    if (coins >= 50) {
      fetch(`https://anchor-full-stack.onrender.com/update-coins?email=${localStorage.getItem('email')}&coins=${coins-50}`, {
        method: 'POST',
      })
        .then((res) => res.json())
        .then((data) => {
